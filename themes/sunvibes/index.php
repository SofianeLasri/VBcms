...
<head>
	...
	?>
	<!-- march po :(
	<script src="themes/<?=getParameters("theme")?>/assets/js/vunit-0.2.0.min.js"></script>
	<script>
	    new vUnit({
	      CSSMap: {
	        '.vh': {
...
<body>
	...
	<img class="topImage" src="themes/<?=getParameters("theme")?>/assets/images/top.png">
	<img class="serverLogo" src="<?=getParameters("serverLogo")?>">


	<div class="loadingPosts">
		<img id="postImage" src="">
		<div id="postText" class="postText"></div>
	</div>
	...
	<script type="text/javascript">
	    ...
	    <?php
	    if ($oldWebBrowser == true) { ?>
	    $("#postText").html("<h1>Hep hep hep!</h1><p>Il semblerait que tu n'utilises pas la Bêta Chromium de Garry's Mod!<br>Le naviguateur intégré est obsolète et ertains éléments risquent de en pas s'afficher correctement. Penses à faire le changement!</p>");
	    $("#postImage").attr("src", "themes/<?=getParameters("theme")?>/assets/images/postAwesomium.png");
	    <?php } else { ?>
	    $("#postText").html(loadingPostsText[0]);
	    $("#postImage").attr("src", loadingPostsImages[0]);
	    <?php } ?>
	    ...
	    setInterval(function () {
            ...
            $(".loadingPosts").fadeOut("slow", function () {
                $("#postText").html(loadingPostsText[c]);
    			$("#postImage").attr("src", loadingPostsImages[c]);
                $(this).fadeIn("slow");
                width = 0;
    			$("#postBar").css("width", width + "%");
            });
